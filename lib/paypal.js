...
async function verifyWebhookSignature({ transmission_id, transmission_time, cert_url, auth_algo, transmission_sig, webhook_id, webhook_event }) {
  if (!transmission_id || !transmission_time || !transmission_sig || !webhook_id) {
    throw new Error('missing_paypal_verification_parameters');
  }
  const token = await getAccessToken();
  const url = DEFAULT_PAYPAL_API + '/v1/notifications/verify-webhook-signature';
  const payload = {
  ...
  const json = await res.json().catch(() => null);
  if (!res.ok) {
    const err = new Error('PayPal verify webhook error');
    err.status = res.status;
    err.response = json;
    throw err;
  }

  // PayPal responds with { verification_status: 'SUCCESS' } on success
  return String(json && json.verification_status || '').toUpperCase() === 'SUCCESS';
}
